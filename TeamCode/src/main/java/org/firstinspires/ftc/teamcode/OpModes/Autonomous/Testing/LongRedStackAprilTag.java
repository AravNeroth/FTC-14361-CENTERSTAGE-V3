...
public class LongRedStackAprilTag extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        TrajectorySequence underTruss = drive.trajectorySequenceBuilder(centerTape.end())
                .setVelConstraint(SampleMecanumDrive.getVelocityConstraint(44, 45, DriveConstants.TRACK_WIDTH))
                .lineToConstantHeading(new Vector2d(30, -59.25))
                .addTemporalMarker(() -> {
...
//                .lineToConstantHeading(new Vector2d(50, -61))
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (currentState) {
                case tape:
                    if(!cameraOn){
                        if(bot.voltMecanum.getBatteryVoltage() < 13.8){
                            batteryOffset = 1;
                        }
                        // newColorDetect();
...
//                                }
                            ...
                            if (tagFound) {
                                ...
                                tag = drive.trajectorySequenceBuilder(drive.getPoseEstimate())
                                        .setVelConstraint(SampleMecanumDrive.getVelocityConstraint(25, 45, DriveConstants.TRACK_WIDTH))
                                        .waitSeconds(.15)
                                        .lineToConstantHeading(new Vector2d(drive.getPoseEstimate().getX() + tagOfInterest.ftcPose.y -2, tagY - 3))
                                        .addDisplacementMarker( 1, () -> {
                                            bot.outtakeSlide.setPosition(650);
                                        })
                                        .addTemporalMarker( () -> {
